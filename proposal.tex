...
Users might not know to exactly data is processed and stored as long as it works.
In spite of that, users need to be sure that their data is stored somewhere
reliably and in a case of emergency, such as a disk failure, the data will be available later
and not lost.

I was 30? years since Berkley invented database management system.
...
Here we're talking about data replication in clickhouse\cite{clickhouse} in parlicular.
...
always a severe problem. The problem of node coordination reduces to a consensus
problem. The consesus problem is solved using concensus protocol. The first consensus
protocol which was proven and explained is Paxos

Clickhouse relies on zookeeper
